void Main()
{
    ...
    texts.push_back("焦躁と言おうか、嫌悪と言おうか――酒を飲んだあとに宿酔ふつかよいがあるように、酒を毎日飲んでいると宿酔に相当した時期がやって来る。それが来たのだ。");
    ...
    texts.push_back("何故だかその頃私は見すぼらしくて美しいものに強くひきつけられたのを覚えている。風景にしても壊れかかった街だとか、その街にしてもよそよそしい表通りよりもどこか親しみのある、");
    ...
    while (System::Update())
    {
		font("ゆっくり実況テスト").drawAt(
            Window::Width() / 2,
            (Window::Height() - 140) / 2,
            Palette::White
            );

        Rect talkRect(0, 326, Window::Width(), 140);

        if (talkRect.leftClicked()) {
            isLeft = !isLeft;
            isSpeech = true;
            index++;
        }
        ...
        if (isLeft) {
            ...
            if (isSpeech) {
                TextToSpeech::Speak(texts[index]);
                isSpeech = false;
            }
